...
if "concurrent.futures" not in text:
    text = text.replace("import time\n", "import time\n" + import_statement)

# Replace research loop
research_match = re.search(r'(try:\n\s*for sub_question, is_cached in cache_hits\.items\(\):\n\s*if not is_cached:.*?)(?=\n\s*research_time =)', text, re.DOTALL)
if research_match:
    old_research = research_match.group(1)
    new_research = """try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for future in concurrent.futures.as_completed(tasks):
                ...
                if status == "success":
                    ...
                    sub_answers[sub_question] = answer"""
    text = text.replace(old_research, new_research)


# Replace eval loop
eval_match = re.search(r'(try:\n\s*for sub_question in sub_questions:\n\s*# Skip cached answers.*?)(?=\n\s*evaluation_time =)', text, re.DOTALL)
if eval_match:
    old_eval = eval_match.group(1)
    new_eval = """try:
