...
def analyze_agent_results(results):
    ...
    ax2.bar(x_pos + width/2, research_calls, width, label='DeepSeek-V3 (Research)', color='lightgreen')
    ...
    # Add exact timing labels on top of the bars
    for i, (cache_lat, research_lat) in enumerate(zip(cache_latencies, research_latencies)):
        # Because it's log scale, placing text needs some space above the bar
...
"""
    ...
    logger.info(f"📊 Visualization successfully saved to {save_path}")
    # plt.show() # Commented out to prevent blocking the script
